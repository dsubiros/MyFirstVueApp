  ...
  <div id="todo-list">
    ...
    <div class="item-list">
      <form @submit.prevent="onSubmit">
        <input type="text" placeholder="What needs to be done?" v-model="text">
      </form>
      ...
      <ul>
        <transition-group enter-active-class="animated bounceInUp" leave-active-class="animated bounceOutDown" name="list">
          <li v-for="(todo, index) in todos" :key="index" title="Click to change item status" 
...
export default class StudentList extends Vue {
  ...
  async getTodos(): Promise<void> {
    this.todos = (await axios.get('https://localhost:5003/api/todos'))?.data || [];
  ...
  async onSubmit() {
    ...
    try {
      ...
      const data: Todo = (await axios.post('https://localhost:5003/api/todos', newTodo))?.data;
      this.text = '';
  ...
  async toggleStatus(index: number) {
    ...
    try {
      const result: Todo = (await axios.put(`https://localhost:5003/api/todos/${item.todoId}`, item))?.data;
